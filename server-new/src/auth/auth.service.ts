...
export class AuthService {
  ...
  generateRefreshToken(payload: { userId: string; sessionId: string; role: string }) {
  ...
  async getUserActivationById(userId: string) {
    const [activation] = await db.select().from(emailActivationsTable)
      .where(eq(emailActivationsTable.userId, userId));
    return activation || null;
  }

  async createOrGetEmailActivation(userId: string) {
    const [activation] = await db.insert(emailActivationsTable)
      .values({ userId })
      .onConflictDoNothing()
      .returning();
    return activation;
  ...
  async deleteUserActivationToken(userId: string) {
    await db.delete(emailActivationsTable)
      .where(eq(emailActivationsTable.userId, userId));
  }

  async getResetPasswordRequest(userId: string) {
    const [request] = await db.select().from(resetPasswordRequestsTable)
      .where(eq(resetPasswordRequestsTable.userId, userId));
    return request || null;
  }

  async createOrGetResetPasswordRequest(userId: string) {
    const [request] = await db.insert(resetPasswordRequestsTable)
      .values({ userId })
      .onConflictDoNothing()
      .returning();
    return request;
  ...
  async deleteUserResetPasswordRequest(userId: string) {
    await db.delete(resetPasswordRequestsTable)
      .where(eq(resetPasswordRequestsTable.userId, userId));
  }
  ...
  async createPasswordAccount(data: { userId: string; hashedPassword: string; }) {
    const { userId, hashedPassword } = data;
    await db.insert(passwordAccountsTable)
  ...
  async changePassword(userId: string, newPassword: string) {
    await db.update(passwordAccountsTable)
