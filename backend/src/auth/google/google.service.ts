...
import { googleAccountsTable, usersTable } from "@db/schema.js";
import { Database, Transaction, HTTPError } from "@/utils.js";
...
export class AuthGoogleService {
	...
	private async processGoogleUser(googleUser: GoogleUserInfo): Promise<string> {
		return this.db.transaction(async (tx: Transaction) => {
			...
			if (existingUser) {
				const googleAccounts = await tx
					.select()
					.from(googleAccountsTable)
					.where(eq(googleAccountsTable.googleId, googleUser.sub));
				...
				if (googleAccount) {
					...
					await Promise.all([
						!existingUser.avatarKey && googleUser.picture
							? this.safePutUserAvatarFromUrl(googleUser.picture, avatarKey)
							: null,
						tx
							.update(usersTable)
							.set({
								avatarKey,
								emailVerified:
									googleUser.email_verified === true ? true : undefined,
							})
	...
	private async safePutUserAvatarFromUrl(
		pictureUrl: string,
		avatarKey: string
	): Promise<void> {
		try {
			...
			if (!res.ok) {
				console.error(
					`Failed to fetch image from ${pictureUrl}: ${res.statusText}`
				);
				return;
			}
