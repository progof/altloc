...
<template>
  <DialogRoot v-model:open="isOpen" :modal="true">
    <DialogTrigger as-child>
      <BurgerToggle
        v-model="isOpen"
        class="p-1.5 text-stone-200 hover:text-zinc-700 md:hidden"
      />
    </DialogTrigger>
    <DialogPortal>
      <DialogOverlay />
      <DialogContent
        ...
        class="h-[calc(100vh - 60px)] fixed inset-y-0 left-0 top-[60px] z-10 w-screen gap-4 bg-indigo-200 px-6 backdrop-blur ease-out data-[state=closed]:duration-300 data-[state=open]:duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:slide-out-to-bottom-2 data-[state=open]:slide-in-from-bottom-2 md:px-10"
      >
        <div class="flex flex-col p-3 gap-6">
          <RouterLink to="/dashboard" class="text-zinc-700 flex gap-3">
            <DashboardIcon class="h-5 w-5" />
            Dashboard
          </RouterLink>
          <RouterLink to="/spaces" class="text-zinc-700 flex gap-3">
            <SpacesIcon class="h-5 w-5" />
            Spaces
          </RouterLink>
          <RouterLink to="/feed" class="text-zinc-700 flex gap-3">
            <FeedIcon class="h-5 w-5" />
            Feed
          </RouterLink>
          <RouterLink to="#" class="text-zinc-700 flex gap-3">
          ...
          <button class="text-zinc-700 flex gap-3" @click="logoutUser">
